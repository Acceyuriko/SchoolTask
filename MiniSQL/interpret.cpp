	...
	if(Catalog.IsTableExists(m_tabname))
	{
		...
		while(tempcond)
		{
			...
			if(type == INT && tempcond->type  == NOTCHAR)
			{
				...
				{
					printf("1Error: data Type error with attribute '%s'! Please check your input!\n", tempcond ->attrname); 
					return;
				}
				tempcond -> type = INT;
			}
			else if(type == FLOAT && tempcond -> type == NOTCHAR)
			{
				...
				{
					printf("2Error: data Type error with attribute '%s'! Please check your input!\n", tempcond ->attrname); 
					return;
				}
				tempcond -> type = FLOAT;
			}
			...
			{
					printf("3Error: data Type error with attribute '%s'! Please check your input!\n", tempcond ->attrname); 
					return;
			}
